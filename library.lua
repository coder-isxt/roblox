...
local UILibrary = (function()
    ...
    local function CleanRegistries()
        for i = #ThemeRegistry, 1, -1 do if not ThemeRegistry[i].Instance or not ThemeRegistry[i].Instance.Parent then table.remove(ThemeRegistry, i) end end
        for i = #FontRegistry, 1, -1 do if not FontRegistry[i].Instance or not FontRegistry[i].Instance.Parent then table.remove(FontRegistry, i) end end
        for i = #CornerRegistry, 1, -1 do if not CornerRegistry[i].Instance or not CornerRegistry[i].Instance.Parent then table.remove(CornerRegistry, i) end end
    end
    ...
    local function UpdateTheme(themeName)
        ...
        for _, item in ipairs(ThemeRegistry) do
            if item.Instance and item.Instance.Parent then
                TweenService:Create(item.Instance, TweenInfo.new(0.3), {[item.Property] = themeColors[item.Role]}):Play()
            end
        end
    ...
    function UILibrary:CreateWindow(title)
        ...
        CreateElement("Frame", { Parent = MainFrame, BorderSizePixel = 0, Position = UDim2.new(0, 150, 0, 40), Size = UDim2.new(0, 1, 1, -40), ZIndex = 5 }, {BackgroundColor3 = "Stroke"})
        ...
        local function SwitchSettingsTab(name)
            for n, tab in pairs(SettingsTabs) do
                ...
                if n == name then
                    TweenService:Create(tab.Button, TweenInfo.new(0.2), {TextColor3 = Themes[Options.Theme].Text, BackgroundTransparency = 0.9}):Play()
                else
                    TweenService:Create(tab.Button, TweenInfo.new(0.2), {TextColor3 = Themes[Options.Theme].SubText, BackgroundTransparency = 1}):Play()
                end
            end
        end
        ...
        local function CreateSettingsButton(text, callback)
            ...
            button.MouseEnter:Connect(function() TweenService:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Hover}):Play() end)
            button.MouseLeave:Connect(function() TweenService:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].TerBg}):Play() end)
            button.MouseButton1Click:Connect(function() pcall(callback); TweenService:Create(button, TweenInfo.new(0.1), {Size = UDim2.new(1, -2, 0, 38)}):Play(); task.wait(0.1); TweenService:Create(button, TweenInfo.new(0.1), {Size = UDim2.new(1, 0, 0, 40)}):Play() end)
        end
            ...
            dropdownButton.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    for i, opt in ipairs(options) do
                        ...
                        optBtn.MouseEnter:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Hover}):Play() end)
                        optBtn.MouseLeave:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].QuarBg}):Play() end)
                        optBtn.MouseButton1Click:Connect(function() dropdownButton.Text = opt; isOpen = false; TweenService:Create(dropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 45)}):Play(); task.delay(0.2, function() if optionContainer then optionContainer:Destroy() end end); pcall(callback, opt) end)
                    end
                    TweenService:Create(dropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 45 + (#options * 30) + 10)}):Play()
                else
                    TweenService:Create(dropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 45)}):Play(); task.delay(0.2, function() if optionContainer then optionContainer:Destroy() end end)
                end
            end)
        end
        ...
        local function CreateSettingsToggle(text, callback)
            ...
            local function syncVisuals(themeUpdate)
                ...
                if Options.ToggleStyle == "Switch" then
                    ...
                    TweenService:Create(switchBg, TweenInfo.new(duration), {BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].QuarBg}):Play()
                    TweenService:Create(switchCircle, TweenInfo.new(duration), {Position = toggled and UDim2.new(1, -20, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), BackgroundColor3 = toggled and Themes[Options.Theme].Text or Themes[Options.Theme].SubText}):Play()
                else
                    switchBg.Visible = false; checkBg.Visible = true
                    TweenService:Create(checkBg, TweenInfo.new(duration), {BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].QuarBg}):Play()
                    TweenService:Create(checkInner, TweenInfo.new(duration), {Size = toggled and UDim2.new(1, -6, 1, -6) or UDim2.new(0, 0, 0, 0)}):Play()
                end
                if not themeUpdate then TweenService:Create(stroke, TweenInfo.new(duration), {Color = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].Stroke}):Play() else stroke.Color = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].Stroke end
            end
            ...
            toggleButton.MouseEnter:Connect(function() TweenService:Create(toggleButton, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Hover}):Play() end)
            toggleButton.MouseLeave:Connect(function() TweenService:Create(toggleButton, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].TerBg}):Play() end)
            syncVisuals(true)
        end
        ...
        local function MinimizeUI()
            ...
            TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = Minimized and UDim2.new(0, 600, 0, 40) or UDim2.new(0, 600, 0, 450)}):Play()
        end
        ...
        local function ToggleUI()
            ...
            if UIVisible then MainFrame.Visible = true; TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 600, 0, 450)}):Play()
            else TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Size = UDim2.new(0, 0, 0, 0)}):Play(); task.delay(0.3, function() if not UIVisible then MainFrame.Visible = false end end) end
        end
        ...
        function window:SwitchToTab(tabToSelect)
            for _, tab in pairs(tabs) do
                ...
                TweenService:Create(tab.Button, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].SecBg, TextColor3 = Themes[Options.Theme].SubText}):Play()
            end
            tabToSelect.Page.Visible = true
            TweenService:Create(tabToSelect.Button, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Accent, TextColor3 = Themes[Options.Theme].Text}):Play()
        end
        ...
        function window:CreateTab(name)
            ...
            tabButton.MouseEnter:Connect(function() if not page.Visible then TweenService:Create(tabButton, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Hover}):Play() end end)
            tabButton.MouseLeave:Connect(function() if not page.Visible then TweenService:Create(tabButton, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].SecBg}):Play() end end)
            ...
            function tab:CreateButton(text, callback)
                ...
                button.MouseEnter:Connect(function() TweenService:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Hover}):Play(); TweenService:Create(buttonStroke, TweenInfo.new(0.2), {Color = Themes[Options.Theme].Accent}):Play() end)
                button.MouseLeave:Connect(function() TweenService:Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].TerBg}):Play(); TweenService:Create(buttonStroke, TweenInfo.new(0.2), {Color = Themes[Options.Theme].Stroke}):Play() end)
                button.MouseButton1Click:Connect(function() pcall(callback); TweenService:Create(button, TweenInfo.new(0.1), {Size = UDim2.new(1, -4, 0, 31)}):Play(); task.wait(0.1); TweenService:Create(button, TweenInfo.new(0.1), {Size = UDim2.new(1, 0, 0, 35)}):Play() end)
                return button
            end
            ...
            function tab:CreateToggle(text, callback)
                ...
                local function syncVisuals(themeUpdate)
                    ...
                    if Options.ToggleStyle == "Switch" then
                        switchBg.Visible = true; checkBg.Visible = false
                        TweenService:Create(switchBg, TweenInfo.new(duration), {BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].QuarBg}):Play()
                        TweenService:Create(switchIndicator, TweenInfo.new(duration), {Position = toggled and UDim2.new(1, -18, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), BackgroundColor3 = toggled and Themes[Options.Theme].Text or Themes[Options.Theme].SubText}):Play()
                    else
                        switchBg.Visible = false; checkBg.Visible = true
                        TweenService:Create(checkBg, TweenInfo.new(duration), {BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].QuarBg}):Play()
                        TweenService:Create(checkInner, TweenInfo.new(duration), {Size = toggled and UDim2.new(1, -6, 1, -6) or UDim2.new(0, 0, 0, 0)}):Play()
            ...
            function tab:CreateKeybind(text, callback)
                ...
                keybindButton.MouseButton1Click:Connect(function()
                    ...
                    waiting = true; keybindButton.Text = "..."; TweenService:Create(keybindStroke, TweenInfo.new(0.2), {Color = Themes[Options.Theme].Accent}):Play()
                    local connection; connection = UserInputService.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.Keyboard then
                            currentKey = input.KeyCode; keybindButton.Text = currentKey.Name; task.delay(0.2, function() waiting = false end)
                            TweenService:Create(keybindStroke, TweenInfo.new(0.2), {Color = Themes[Options.Theme].Stroke}):Play(); connection:Disconnect()
                        end
                    end)
                end)
                local lastInput = 0
            ...
            function tab:CreateCycleButton(text, values, default, callback)
                ...
                cycleButton.MouseButton1Click:Connect(function()
                    ...
                    task.wait(0.1); TweenService:Create(cycleButton, TweenInfo.new(0.1), {Size = UDim2.new(0, 100, 0, 22)}):Play()
                end)
                ...
                dropdownButton.MouseButton1Click:Connect(function()
                    ...
                    if isOpen then
                        ...
                        for i, opt in ipairs(options) do
                            ...
                            CreateElement("UICorner", {CornerRadius = UDim.new(0, 6), Parent = optBtn})
                            optBtn.MouseEnter:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].Hover}):Play() end)
                            optBtn.MouseLeave:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.2), {BackgroundColor3 = Themes[Options.Theme].TerBg}):Play() end)
                            optBtn.MouseButton1Click:Connect(function() dropdownButton.Text = opt; isOpen = false; TweenService:Create(dropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 45)}):Play(); task.delay(0.2, function() if optionContainer then optionContainer:Destroy() end end); pcall(callback, opt) end)
                        end
                        TweenService:Create(dropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 45 + (#options * 30) + 10)}):Play()
                    else
