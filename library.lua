...
local UILibrary = (function()
    ...
    local function CleanRegistries()
        ...
        for i = #FontRegistry, 1, -1 do if not FontRegistry[i].Instance or not FontRegistry[i].Instance.Parent then table
                    .remove(FontRegistry, i) end end
        for i = #CornerRegistry, 1, -1 do if not CornerRegistry[i].Instance or not CornerRegistry[i].Instance.Parent then
                table.remove(CornerRegistry, i) end end
    end
    ...
    local function CreateElement(class, properties, themeData)
        ...
        if properties.Font then
            ...
            if properties.Font == Enum.Font.GothamBold then
                weight = "Bold"
            elseif properties.Font == Enum.Font.GothamBlack then
                weight = "Black"
            end
            table.insert(FontRegistry, { Instance = element, Weight = weight })
        ...
        if themeData then
            for prop, role in pairs(themeData) do
                table.insert(ThemeRegistry, { Instance = element, Property = prop, Role = role })
    ...
    local function UpdateCornerStyle(styleName)
        ...
        for _, item in ipairs(CornerRegistry) do
            if item.Instance and item.Instance.Parent then
                ...
                if styleName == "Blocky" then
                    newRadius = UDim.new(0, 0)
                elseif styleName == "Slight" then
                    if item.Original.Scale ~= 1 then newRadius = UDim.new(0, math.floor(item.Original.Offset / 2)) end
                end
                TweenService:Create(item.Instance, TweenInfo.new(0.3), { CornerRadius = newRadius }):Play()
            end
        end
    end
    ...
    function UILibrary:CreateWindow(title)
        ...
        CreateElement("Frame",
            { Parent = TabContainer, BorderSizePixel = 0, Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(1, 0, 0, 10) },
            { BackgroundColor3 = "SecBg" })
        CreateElement("Frame",
            { Parent = TabContainer, BorderSizePixel = 0, Position = UDim2.new(1, -10, 0, 0), Size = UDim2.new(0, 10, 1,
                0) }, { BackgroundColor3 = "SecBg" })

        local TabHolder = CreateElement("ScrollingFrame",
            { Name = "TabHolder", Parent = TabContainer, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), CanvasSize =
            UDim2.new(0, 0, 0, 0), ScrollBarThickness = 0, BorderSizePixel = 0, AutomaticCanvasSize = Enum.AutomaticSize
            .Y })
        ...
        local SettingsOverlay = CreateElement("Frame",
            { Name = "SettingsOverlay", Parent = MainFrame, BackgroundColor3 = Color3.fromRGB(0, 0, 0), BackgroundTransparency = 1, Size =
            UDim2.new(1, 0, 1, 0), Visible = false, ZIndex = 20 })

        local SettingsFrame = CreateElement("Frame",
            { Name = "SettingsFrame", Parent = SettingsOverlay, BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5, 0.5), Position =
            UDim2.new(0.5, 0, 0.5, 10), Size = UDim2.new(0, 500, 0, 350), ClipsDescendants = true },
            { BackgroundColor3 = "SecBg" })
        ...
        local SettingsHeader = CreateElement("Frame",
            { Parent = SettingsFrame, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 40) })
        CreateElement("TextLabel",
            { Parent = SettingsHeader, BackgroundTransparency = 1, Position = UDim2.new(0, 20, 0, 0), Size = UDim2.new(1,
                -50, 1, 0), Font = Enum.Font.GothamBold, Text = "Settings", TextSize = 18, TextXAlignment = Enum
            .TextXAlignment.Left }, { TextColor3 = "Text" })
        local CloseSettingsButton = CreateElement("TextButton",
            { Parent = SettingsHeader, BackgroundTransparency = 1, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2
            .new(1, -15, 0.5, 0), Size = UDim2.new(0, 24, 0, 24), Font = Enum.Font.GothamBold, Text = "X", TextSize = 16 },
            { TextColor3 = "SubText" })

        local SettingsBody = CreateElement("Frame",
            { Parent = SettingsFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 0, 0, 40), Size = UDim2.new(1,
                0, 1, -40) })
        local SettingsSidebar = CreateElement("ScrollingFrame",
            { Parent = SettingsBody, BackgroundTransparency = 1, Size = UDim2.new(0, 140, 1, 0), CanvasSize = UDim2.new(
            0, 0, 0, 0), ScrollBarThickness = 0 })
        CreateElement("UIListLayout",
            { Parent = SettingsSidebar, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 5) })
        CreateElement("UIPadding",
            { Parent = SettingsSidebar, PaddingTop = UDim.new(0, 10), PaddingLeft = UDim.new(0, 10) })
        CreateElement("Frame",
            { Parent = SettingsBody, BorderSizePixel = 0, Position = UDim2.new(0, 140, 0, 10), Size = UDim2.new(0, 1, 1,
                -20) }, { BackgroundColor3 = "Stroke" })

        local SettingsContent = CreateElement("Frame",
            { Parent = SettingsBody, BackgroundTransparency = 1, Position = UDim2.new(0, 150, 0, 0), Size = UDim2.new(1,
                -150, 1, 0) })
            ...
            for n, tab in pairs(SettingsTabs) do
                ...
                if n == name then
                    TweenService:Create(tab.Button, TweenInfo.new(0.2),
                        { TextColor3 = Themes[Options.Theme].Text, BackgroundTransparency = 0.9 }):Play()
                else
                    TweenService:Create(tab.Button, TweenInfo.new(0.2),
                        { TextColor3 = Themes[Options.Theme].SubText, BackgroundTransparency = 1 }):Play()
                end
            end
        end
        ...
        local function CreateSettingsSection(text)
            ...
            local tabBtn = CreateElement("TextButton",
                { Parent = SettingsSidebar, BackgroundTransparency = 1, BackgroundColor3 = Color3.fromRGB(255, 255, 255), Size =
                UDim2.new(1, -10, 0, 30), Text = text, Font = Enum.Font.GothamBold, TextSize = 13, TextXAlignment = Enum
                .TextXAlignment.Left, AutoButtonColor = false }, { TextColor3 = "SubText" })
            ...
            local page = CreateElement("ScrollingFrame",
                { Parent = SettingsContent, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Visible = false, CanvasSize =
                UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2, AutomaticCanvasSize = Enum.AutomaticSize.Y })
            CreateElement("UIListLayout",
                { Parent = page, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 8) })
            CreateElement("UIPadding",
                { Parent = page, PaddingTop = UDim.new(0, 10), PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0,
                    10), PaddingBottom = UDim.new(0, 10) })
        ...
        local function CreateSettingsButton(text, callback)
            local button = CreateElement("TextButton",
                { Parent = CurrentSettingsPage, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 40), AutoButtonColor = false, Text =
            ...
            CreateElement("TextLabel",
                { Parent = button, BackgroundTransparency = 1, Position = UDim2.new(0, 15, 0, 0), Size = UDim2.new(1, -15,
                    1, 0), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum.TextXAlignment
                .Left }, { TextColor3 = "Text" })
            button.MouseEnter:Connect(function() TweenService:Create(button, TweenInfo.new(0.2),
                    { BackgroundColor3 = Themes[Options.Theme].Hover }):Play() end)
            button.MouseLeave:Connect(function() TweenService:Create(button, TweenInfo.new(0.2),
                    { BackgroundColor3 = Themes[Options.Theme].TerBg }):Play() end)
            button.MouseButton1Click:Connect(function()
                pcall(callback); TweenService:Create(button, TweenInfo.new(0.1), { Size = UDim2.new(1, -2, 0, 38) })
                    :Play(); task.wait(0.1); TweenService:Create(button, TweenInfo.new(0.1),
        ...
        local function CreateSettingsDropdown(text, options, default, callback)
            local dropdownFrame = CreateElement("Frame",
                { Parent = CurrentSettingsPage, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 45), ClipsDescendants = true },
                { BackgroundColor3 = "TerBg" })
            ...
            CreateElement("TextLabel",
                { Parent = dropdownFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 15, 0, 0), Size = UDim2
                .new(0.5, 0, 0, 45), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                .TextXAlignment.Left }, { TextColor3 = "Text" })

            local dropdownButton = CreateElement("TextButton",
                { Parent = dropdownFrame, BorderSizePixel = 0, AnchorPoint = Vector2.new(1, 0), Position = UDim2.new(1,
                    -15, 0, 8), Size = UDim2.new(0, 120, 0, 28), Font = Enum.Font.GothamBold, Text = default or
                options[1] or "None", TextSize = 12, AutoButtonColor = false },
            ...
            dropdownButton.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    optionContainer = CreateElement("Frame",
                        { Parent = dropdownFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 0, 0, 45), Size =
                        UDim2.new(1, 0, 0, #options * 30) })
                    for i, opt in ipairs(options) do
                        local optBtn = CreateElement("TextButton",
                            { Parent = optionContainer, BorderSizePixel = 0, Position = UDim2.new(0, 10, 0, (i - 1) * 30), Size =
                            UDim2.new(1, -20, 0, 28), Font = Enum.Font.Gotham, Text = opt, TextSize = 12, AutoButtonColor = false },
                            { BackgroundColor3 = "QuarBg", TextColor3 = "Text" })
                        ...
                        optBtn.MouseButton1Click:Connect(function()
                            dropdownButton.Text = opt; isOpen = false; TweenService:Create(dropdownFrame,
                                TweenInfo.new(0.2), { Size = UDim2.new(1, 0, 0, 45) }):Play(); task.delay(0.2,
                                function() if optionContainer then optionContainer:Destroy() end end); pcall(callback,
                                opt)
                        end)
                    end
                    TweenService:Create(dropdownFrame, TweenInfo.new(0.2),
                        { Size = UDim2.new(1, 0, 0, 45 + (#options * 30) + 10) }):Play()
                else
                    TweenService:Create(dropdownFrame, TweenInfo.new(0.2), { Size = UDim2.new(1, 0, 0, 45) }):Play(); task
                        .delay(0.2, function() if optionContainer then optionContainer:Destroy() end end)
                end
            end)
        end
        ...
        CreateSettingsDropdown("Corner Style", { "Rounded", "Slight", "Blocky" }, Options.CornerStyle,
            function(val) UpdateCornerStyle(val) end)
        CreateSettingsDropdown("Global Font", { "Gotham", "Ubuntu", "Code", "Jura" }, Options.Font,
            function(val) UpdateFont(val) end)

        CreateSettingsSection("General")
        local function CreateSettingsToggle(text, callback)
            local toggleButton = CreateElement("TextButton",
                { Parent = CurrentSettingsPage, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 45), AutoButtonColor = false, Text =
                "" }, { BackgroundColor3 = "TerBg" })
            ...
            CreateElement("TextLabel",
                { Parent = toggleButton, BackgroundTransparency = 1, Position = UDim2.new(0, 15, 0, 0), Size = UDim2.new(
                0.7, 0, 1, 0), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                .TextXAlignment.Left }, { TextColor3 = "Text" })

            local toggleContainer = CreateElement("Frame",
                { Parent = toggleButton, BackgroundTransparency = 1, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2
                .new(1, -15, 0.5, 0), Size = UDim2.new(0, 44, 0, 22) })

            local switchBg = CreateElement("Frame",
                { Parent = toggleContainer, BorderSizePixel = 0, Size = UDim2.new(1, 0, 1, 0) },
            ...
            local switchCircle = CreateElement("Frame",
                { Parent = switchBg, BorderSizePixel = 0, AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 2,
                    0.5, 0), Size = UDim2.new(0, 18, 0, 18) }, { BackgroundColor3 = "SubText" })
            CreateElement("UICorner", { CornerRadius = UDim.new(1, 0), Parent = switchCircle })

            local checkBg = CreateElement("Frame",
                { Parent = toggleContainer, BorderSizePixel = 0, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(
                1, 0, 0.5, 0), Size = UDim2.new(0, 22, 0, 22), Visible = false }, { BackgroundColor3 = "QuarBg" })
            CreateElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = checkBg })
            local checkInner = CreateElement("Frame",
                { Parent = checkBg, BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0,
                    0.5, 0), Size = UDim2.new(0, 0, 0, 0) }, { BackgroundColor3 = "Accent" })
            ...
            local function syncVisuals(themeUpdate)
                ...
                if Options.ToggleStyle == "Switch" then
                    ...
                    TweenService:Create(switchBg, TweenInfo.new(duration),
                        { BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].QuarBg })
                        :Play()
                    TweenService:Create(switchCircle, TweenInfo.new(duration),
                        { Position = toggled and UDim2.new(1, -20, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), BackgroundColor3 =
                        toggled and Themes[Options.Theme].Text or Themes[Options.Theme].SubText }):Play()
                else
                    switchBg.Visible = false; checkBg.Visible = true
                    TweenService:Create(checkBg, TweenInfo.new(duration),
                        { BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].QuarBg })
                        :Play()
                    TweenService:Create(checkInner, TweenInfo.new(duration),
                        { Size = toggled and UDim2.new(1, -6, 1, -6) or UDim2.new(0, 0, 0, 0) }):Play()
                end
                if not themeUpdate then TweenService:Create(stroke, TweenInfo.new(duration),
                        { Color = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].Stroke }):Play() else stroke.Color =
                    toggled and Themes[Options.Theme].Accent or Themes[Options.Theme].Stroke end
            end
            table.insert(ToggleRegistry, syncVisuals)
            toggleButton.MouseButton1Click:Connect(function()
                toggled = not toggled; syncVisuals(); pcall(callback, toggled)
            end)
            toggleButton.MouseEnter:Connect(function() TweenService:Create(toggleButton, TweenInfo.new(0.2),
                    { BackgroundColor3 = Themes[Options.Theme].Hover }):Play() end)
            toggleButton.MouseLeave:Connect(function() TweenService:Create(toggleButton, TweenInfo.new(0.2),
                    { BackgroundColor3 = Themes[Options.Theme].TerBg }):Play() end)
            syncVisuals(true)
        end
        ...
        CreateSettingsToggle("Performance Mode (FPS Boost)", function(state)
            if state then
                ...
                FPSCleanup = function()
                    ...
                    for _, v in pairs(workspace:GetDescendants()) do
                        if v:IsA("BasePart") and not v.Parent:FindFirstChild("Humanoid") then
                            v.Material = Enum.Material.Plastic; v.CastShadow = true
                        elseif v:IsA("Texture") or v:IsA("Decal") then
                            v.Transparency = 0
                        elseif v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Smoke") or v:IsA("Fire") or v:IsA("Sparkles") then
                            v.Enabled = true
        ...
        CreateSettingsButton("Load Remotespy",
            function() loadstring(game:HttpGet(
                "https://rawscripts.net/raw/Universal-Script-RemoteSpy-for-Xeno-and-Solara-32578"))() end)
        CreateSettingsButton("Load DevEx",
            function() loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Dex-with-tags-78265"))() end)

        local function ToggleSettings()
            if SettingsOverlay.Visible then
                TweenService:Create(SettingsOverlay, TweenInfo.new(0.2), { BackgroundTransparency = 1 }):Play()
                TweenService:Create(SettingsFrame, TweenInfo.new(0.2),
                    { Position = UDim2.new(0.5, 0, 0.5, 10), BackgroundTransparency = 1 }):Play()
                task.delay(0.2, function() SettingsOverlay.Visible = false end)
            else
                SettingsOverlay.Visible = true; SettingsOverlay.BackgroundTransparency = 1; SettingsFrame.Position =
                UDim2.new(0.5, 0, 0.5, 10); SettingsFrame.BackgroundTransparency = 1
        ...
        SettingsOverlay.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                local pos = input.Position; local framePos = SettingsFrame.AbsolutePosition; local frameSize =
                SettingsFrame.AbsoluteSize
        ...
        CloseButton.MouseEnter:Connect(function() TweenService:Create(CloseButton, TweenInfo.new(0.2),
                { BackgroundColor3 = Color3.fromRGB(255, 100, 100) }):Play() end)
        CloseButton.MouseLeave:Connect(function() TweenService:Create(CloseButton, TweenInfo.new(0.2),
                { BackgroundColor3 = Color3.fromRGB(255, 80, 80) }):Play() end)
        ...
        local function ToggleUI()
            ...
            if UIVisible then
                MainFrame.Visible = true; TweenService:Create(MainFrame,
                    TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                    { Size = UDim2.new(0, 600, 0, 450) }):Play()
            else
                TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In),
                    { Size = UDim2.new(0, 0, 0, 0) }):Play(); task.delay(0.3,
                    function() if not UIVisible then MainFrame.Visible = false end end)
            end
        end
        ...
        CollapseKeybindButton.MouseButton1Click:Connect(function()
            ...
            local connection; connection = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    collapseKey = input.KeyCode; local keyName = input.KeyCode.Name; if #keyName > 5 then keyName =
                        keyName:sub(1, 4) end
        ...
        function window:SwitchToTab(tabToSelect)
            for _, tab in pairs(tabs) do
                ...
                TweenService:Create(tab.Button, TweenInfo.new(0.2),
                    { BackgroundColor3 = Themes[Options.Theme].SecBg, TextColor3 = Themes[Options.Theme].SubText }):Play()
            end
            tabToSelect.Page.Visible = true
            TweenService:Create(tabToSelect.Button, TweenInfo.new(0.2),
                { BackgroundColor3 = Themes[Options.Theme].Accent, TextColor3 = Themes[Options.Theme].Text }):Play()
        end
        ...
        function window:CreateTab(name)
            ...
            local tabButton = CreateElement("TextButton",
                ...
                .TextXAlignment.Left }, { BackgroundColor3 = "SecBg", TextColor3 = "SubText" })
            ...
            local page = CreateElement("ScrollingFrame",
                { Name = name .. "Page", Parent = ContentFrame, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), ScrollBarThickness = 2, Visible = false },
                { ScrollBarImageColor3 = "Stroke" })
            CreateElement("UIListLayout",
                { Parent = page, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 8) })
            CreateElement("UIPadding",
                { Parent = page, PaddingRight = UDim.new(0, 10), PaddingLeft = UDim.new(0, 5), PaddingTop = UDim.new(0, 5) })

            tab.Button = tabButton; tab.Page = page; table.insert(tabs, tab)
            tabButton.MouseEnter:Connect(function() if not page.Visible then TweenService:Create(tabButton,
                        TweenInfo.new(0.2), { BackgroundColor3 = Themes[Options.Theme].Hover }):Play() end end)
            tabButton.MouseLeave:Connect(function() if not page.Visible then TweenService:Create(tabButton,
                        TweenInfo.new(0.2), { BackgroundColor3 = Themes[Options.Theme].SecBg }):Play() end end)
            tabButton.MouseButton1Click:Connect(function() window:SwitchToTab(tab) end)
            if #tabs == 1 then window:SwitchToTab(tab) end

            function tab:CreateButton(text, callback)
                local button = CreateElement("TextButton",
                    { Parent = page, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 35), Font = Enum.Font.GothamBold, Text =
                    text, TextSize = 14, LayoutOrder = #page:GetChildren() },
                    { BackgroundColor3 = "TerBg", TextColor3 = "Text" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = button })
                local buttonStroke = CreateElement("UIStroke", { Thickness = 1, Transparency = 0, Parent = button },
                    { Color = "Stroke" })

                button.MouseEnter:Connect(function()
                    TweenService:Create(button, TweenInfo.new(0.2), { BackgroundColor3 = Themes[Options.Theme].Hover })
                        :Play(); TweenService:Create(buttonStroke, TweenInfo.new(0.2),
                        { Color = Themes[Options.Theme].Accent }):Play()
                end)
                button.MouseLeave:Connect(function()
                    TweenService:Create(button, TweenInfo.new(0.2), { BackgroundColor3 = Themes[Options.Theme].TerBg })
                        :Play(); TweenService:Create(buttonStroke, TweenInfo.new(0.2),
                        { Color = Themes[Options.Theme].Stroke }):Play()
                end)
                button.MouseButton1Click:Connect(function()
                    pcall(callback); TweenService:Create(button, TweenInfo.new(0.1), { Size = UDim2.new(1, -4, 0, 31) })
                        :Play(); task.wait(0.1); TweenService:Create(button, TweenInfo.new(0.1),
            ...
            function tab:CreateToggle(text, callback)
                local toggleFrame = CreateElement("Frame",
                    { Parent = page, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 35), LayoutOrder = #page:GetChildren() },
                    { BackgroundColor3 = "TerBg" })
                ...
                CreateElement("TextLabel",
                    { Parent = toggleFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 0), Size = UDim2
                    .new(0.7, 0, 1, 0), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                    .TextXAlignment.Left }, { TextColor3 = "Text" })
                local toggleButton = CreateElement("TextButton",
                    { Parent = toggleFrame, BackgroundTransparency = 1, AnchorPoint = Vector2.new(1, 0.5), Position =
                    UDim2.new(1, -8, 0.5, 0), Size = UDim2.new(0, 40, 0, 20), Text = "" })

                local switchBg = CreateElement("Frame",
                    { Parent = toggleButton, BorderSizePixel = 0, Size = UDim2.new(1, 0, 1, 0) },
                    { BackgroundColor3 = "QuarBg" })
                CreateElement("UICorner", { CornerRadius = UDim.new(1, 0), Parent = switchBg })
                local switchIndicator = CreateElement("Frame",
                    { Parent = switchBg, BorderSizePixel = 0, AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0,
                        2, 0.5, 0), Size = UDim2.new(0, 16, 0, 16) }, { BackgroundColor3 = "SubText" })
                CreateElement("UICorner", { CornerRadius = UDim.new(1, 0), Parent = switchIndicator })

                local checkBg = CreateElement("Frame",
                    { Parent = toggleButton, BorderSizePixel = 0, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2
                    .new(1, 0, 0.5, 0), Size = UDim2.new(0, 20, 0, 20), Visible = false }, { BackgroundColor3 = "QuarBg" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = checkBg })
                local checkInner = CreateElement("Frame",
                    { Parent = checkBg, BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(
                    0.5, 0, 0.5, 0), Size = UDim2.new(0, 0, 0, 0) }, { BackgroundColor3 = "Accent" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 3), Parent = checkInner })
                ...
                local function syncVisuals(themeUpdate)
                    ...
                    if Options.ToggleStyle == "Switch" then
                        switchBg.Visible = true; checkBg.Visible = false
                        TweenService:Create(switchBg, TweenInfo.new(duration),
                            { BackgroundColor3 = toggled and Themes[Options.Theme].Accent or Themes[Options.Theme]
                            .QuarBg }):Play()
                        TweenService:Create(switchIndicator, TweenInfo.new(duration),
                            { Position = toggled and UDim2.new(1, -18, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), BackgroundColor3 =
                            toggled and Themes[Options.Theme].Text or Themes[Options.Theme].SubText }):Play()
                    else
                        switchBg.Visible = false; checkBg.Visible = true
                        TweenService:Create(checkBg, TweenInfo.new(duration),
            ...
            function tab:CreateKeybind(text, callback)
                ...
                CreateElement("TextLabel",
                    { Parent = keybindFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 0), Size = UDim2
                    .new(0.6, 0, 1, 0), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                    .TextXAlignment.Left }, { TextColor3 = "Text" })
                local keybindButton = CreateElement("TextButton",
                    { Parent = keybindFrame, BorderSizePixel = 0, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2
                    .new(1, -8, 0.5, 0), Size = UDim2.new(0, 80, 0, 22), Font = Enum.Font.GothamBold, Text = "None", TextSize = 12 },
                    { BackgroundColor3 = "QuarBg", TextColor3 = "Text" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = keybindButton })
                local keybindStroke = CreateElement("UIStroke", { Thickness = 1, Transparency = 0, Parent = keybindButton },
                    { Color = "Stroke" })

                local currentKey = nil; local waiting = false
                keybindButton.MouseButton1Click:Connect(function()
                    if waiting then return end
                    waiting = true; keybindButton.Text = "..."; TweenService:Create(keybindStroke, TweenInfo.new(0.2),
                        { Color = Themes[Options.Theme].Accent }):Play()
                    local connection; connection = UserInputService.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.Keyboard then
                            currentKey = input.KeyCode; keybindButton.Text = currentKey.Name; task.delay(0.2,
                                function() waiting = false end)
                            TweenService:Create(keybindStroke, TweenInfo.new(0.2), { Color = Themes[Options.Theme]
                            .Stroke }):Play(); connection:Disconnect()
                        end
                    end)
                end)
                local lastInput = 0
            ...
            function tab:CreateSlider(text, min, max, default, callback)
                local sliderFrame = CreateElement("Frame",
                    { Parent = page, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 50), LayoutOrder = #page:GetChildren() },
                    { BackgroundColor3 = "TerBg" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = sliderFrame })
                CreateElement("UIStroke", { Thickness = 1, Transparency = 0, Parent = sliderFrame }, { Color = "Stroke" })

                CreateElement("TextLabel",
                    { Parent = sliderFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 5), Size = UDim2
                    .new(0.5, 0, 0.4, 0), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                    .TextXAlignment.Left }, { TextColor3 = "Text" })
                local valueLabel = CreateElement("TextLabel",
                    { Parent = sliderFrame, BackgroundTransparency = 1, Position = UDim2.new(0.5, 0, 0, 5), Size = UDim2
                    .new(0.5, -10, 0.4, 0), Font = Enum.Font.Gotham, Text = tostring(default), TextSize = 12, TextXAlignment =
                    Enum.TextXAlignment.Right }, { TextColor3 = "SubText" })
                local sliderBar = CreateElement("TextButton",
                    { Parent = sliderFrame, BorderSizePixel = 0, Position = UDim2.new(0.025, 0, 0.65, 0), Size = UDim2
                    .new(0.95, 0, 0.15, 0), Text = "" }, { BackgroundColor3 = "QuarBg" })
                CreateElement("UICorner", { CornerRadius = UDim.new(1, 0), Parent = sliderBar })

                local fill = CreateElement("Frame",
                    { Parent = sliderBar, BorderSizePixel = 0, Size = UDim2.new((default - min) / (max - min), 0, 1, 0) },
                ...
                UserInputService.InputChanged:Connect(function(input) if isDragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                        updateSlider(input.Position) end end)
                return sliderFrame
            end
            ...
            function tab:CreateCycleButton(text, values, default, callback)
                ...
                CreateElement("TextLabel",
                    { Parent = cycleFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 0), Size = UDim2
                    .new(0.6, 0, 1, 0), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                    .TextXAlignment.Left }, { TextColor3 = "Text" })
                local cycleButton = CreateElement("TextButton",
                    { Parent = cycleFrame, BorderSizePixel = 0, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(
                    1, -8, 0.5, 0), Size = UDim2.new(0, 100, 0, 22), Font = Enum.Font.GothamBold, Text = tostring(
                    default or values[1] or "None"), TextSize = 12 }, { BackgroundColor3 = "QuarBg", TextColor3 = "Text" })
                ...
                local idx = 1
                for i, v in ipairs(values) do if v == default then
                        idx = i
                        break
                    end end
                local function update()
                    local val = values[idx]; cycleButton.Text = tostring(val); pcall(callback, val)
                end
                ...
                cycleButton.MouseButton1Click:Connect(function()
                    ...
                    task.wait(0.1); TweenService:Create(cycleButton, TweenInfo.new(0.1),
                        { Size = UDim2.new(0, 100, 0, 22) }):Play()
                end)
                return { Frame = cycleFrame, SetValues = function(self, newValues)
                    values = newValues; idx = 1; if #values > 0 then cycleButton.Text = tostring(values[1]) else cycleButton.Text =
                        "None" end
            ...
            function tab:CreateDropdown(text, options, default, callback)
                local dropdownFrame = CreateElement("Frame",
                    { Parent = page, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 45), ClipsDescendants = true, LayoutOrder = #
                    page:GetChildren() }, { BackgroundColor3 = "TerBg" })
                ...
                CreateElement("TextLabel",
                    { Parent = dropdownFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 0), Size = UDim2
                    .new(0.5, 0, 0, 45), Font = Enum.Font.GothamBold, Text = text, TextSize = 14, TextXAlignment = Enum
                    .TextXAlignment.Left }, { TextColor3 = "Text" })

                local dropdownButton = CreateElement("TextButton",
                    { Parent = dropdownFrame, BorderSizePixel = 0, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2
                    .new(1, -10, 0.5, 0), Size = UDim2.new(0, 120, 0, 30), Font = Enum.Font.GothamBold, Text = default or
                    options[1] or "None", TextSize = 12, AutoButtonColor = false },
                    { BackgroundColor3 = "QuarBg", TextColor3 = "SubText" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = dropdownButton })
                CreateElement("UIStroke", { Thickness = 1, Transparency = 0, Parent = dropdownButton },
                    { Color = "Stroke" })
                ...
                dropdownButton.MouseButton1Click:Connect(function()
                    ...
                    if isOpen then
                        ...
                        for i, opt in ipairs(options) do
                            local optBtn = CreateElement("TextButton",
                                { Parent = optionContainer, BorderSizePixel = 0, Position = UDim2.new(0, 10, 0, (i - 1) *
                                30), Size = UDim2.new(1, -20, 0, 28), Font = Enum.Font.Gotham, Text = opt, TextSize = 12, AutoButtonColor = false },
                                { BackgroundColor3 = "TerBg", TextColor3 = "SubText" })
                            CreateElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = optBtn })
                            optBtn.MouseEnter:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.2),
                                    { BackgroundColor3 = Themes[Options.Theme].Hover }):Play() end)
                            optBtn.MouseLeave:Connect(function() TweenService:Create(optBtn, TweenInfo.new(0.2),
                            ...
                            optBtn.MouseButton1Click:Connect(function()
                                dropdownButton.Text = opt; isOpen = false; TweenService:Create(dropdownFrame,
                                    TweenInfo.new(0.2), { Size = UDim2.new(1, 0, 0, 45) }):Play(); task.delay(0.2,
                                    function() if optionContainer then optionContainer:Destroy() end end); pcall(
                                callback, opt)
                            end)
                        end
                        TweenService:Create(dropdownFrame, TweenInfo.new(0.2),
            ...
            function tab:CreateParagraph(title, content)
                local paragraphFrame = CreateElement("Frame",
                    { Parent = page, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum
                    .AutomaticSize.Y, LayoutOrder = #page:GetChildren() }, { BackgroundColor3 = "TerBg" })
                CreateElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = paragraphFrame })
                CreateElement("UIStroke", { Thickness = 1, Transparency = 0, Parent = paragraphFrame },
                    { Color = "Stroke" })
                CreateElement("TextLabel",
                    { Parent = paragraphFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 10), Size =
                    UDim2.new(1, -20, 0, 15), Font = Enum.Font.GothamBold, Text = title, TextSize = 14, TextXAlignment =
                    Enum.TextXAlignment.Left }, { TextColor3 = "Text" })
                CreateElement("TextLabel",
                    { Parent = paragraphFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 30), Size =
                    UDim2.new(1, -20, 0, 0), AutomaticSize = Enum.AutomaticSize.Y, Font = Enum.Font.Gotham, Text =
                    content, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true },
                    { TextColor3 = "SubText" })
        ...
        function UILibrary:Notify(args)
            ...
            if not notificationGui then
                notificationGui = Instance.new("ScreenGui"); notificationGui.Name = "CustomNotificationGui"; notificationGui.Parent =
                game:GetService("CoreGui"); notificationGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; notificationGui.ResetOnSpawn = false
                local container = Instance.new("Frame"); container.Name = "Container"; container.Parent = notificationGui; container.BackgroundTransparency = 1; container.AnchorPoint =
                Vector2.new(1, 0); container.Position = UDim2.new(1, -20, 0, 80); container.Size = UDim2.new(0, 280, 0.5,
                    0)
                local layout = Instance.new("UIListLayout"); layout.Parent = container; layout.SortOrder = Enum
                .SortOrder.LayoutOrder; layout.Padding = UDim.new(0, 10); layout.HorizontalAlignment = Enum
            ...
            CreateElement("TextLabel",
                { Parent = frame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 5), Size = UDim2.new(1, -20,
                    0, 20), Font = Enum.Font.GothamBold, Text = title, TextSize = 16, TextXAlignment = Enum
                .TextXAlignment.Left }, { TextColor3 = "Text" })
            CreateElement("TextLabel",
                { Parent = frame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 25), Size = UDim2.new(1, -20,
                    1, -30), Font = Enum.Font.Gotham, Text = content, TextSize = 14, TextXAlignment = Enum
                .TextXAlignment.Left, TextWrapped = true }, { TextColor3 = "SubText" })
