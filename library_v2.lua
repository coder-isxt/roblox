...
function Window:CreateUniversalCategory(options)
    ...
    local spyState = sharedGlobals.__LIMBO_HYDROXIDE_SPY_STATE
    if type(spyState) ~= "table" then
        spyState = {}
        sharedGlobals.__LIMBO_HYDROXIDE_SPY_STATE = spyState
    end
    UILibrary._remoteSpyState = spyState

    spyState.RemotesViewing = spyState.RemotesViewing or {
        RemoteEvent = true,
    ...
    local getInfoFn = (debug and debug.getinfo) or getinfo or getInfo

    local remoteMethodByClass = {
    ...
    local function cloneArg(v, depth, seen)
        local t = typeof(v)
        if t == "table" then
            depth = depth or 0
            if depth > 6 then
            ...
            for k, val in pairs(v) do
                local ck = cloneArg(k, depth + 1, seen)
                if ck ~= nil then
                    out[ck] = cloneArg(val, depth + 1, seen)
                end
            end
            return out
    ...
    local function cloneArgs(argsPack)
        local out = { n = argsPack.n or 0 }
        for i = 1, out.n do
            out[i] = cloneArg(argsPack[i], 0, {})
        end
        return out
    end
    ...
    local function serializeValue(value, depth, seen)
        ...
        elseif t == "table" then
            ...
            if arrayOnly then
                for i = 1, maxN do
                    parts[#parts + 1] = serializeValue(value[i], depth + 1, seen)
                end
                seen[value] = nil
            ...
            for k, val in pairs(value) do
                ...
                parts[#parts + 1] = keyCode .. " = " .. serializeValue(val, depth + 1, seen)
            end
            table.sort(parts)
            seen[value] = nil
            return "{ " .. table.concat(parts, ", ") .. " }"
    ...
    local function buildCode(remoteInstance, callInfo)
        local method = canonicalMethod(callInfo and callInfo.Method) or remoteMethodByClass[remoteInstance.ClassName] or "FireServer"
        local argsPack = (callInfo and callInfo.Args) or { n = 0 }
        local lines = {
            "-- Generated by Limbo Remotes (Hydroxide-style)",
            "local remote = " .. instancePath(remoteInstance),
            "local args = {",
        }
        ...
        if method == "InvokeServer" or method == "Invoke" then
            lines[#lines + 1] = "    local result = remote:" .. method .. "(unpackFn(args, 1, args.n))"
            lines[#lines + 1] = "    print(\"Remote result:\", result)"
        else
            lines[#lines + 1] = "    remote:" .. method .. "(unpackFn(args, 1, args.n))"
        end
        lines[#lines + 1] = "end"
    ...
    local function newRemoteObject(instance)
        ...
        function remote:Block()
            self.Blocked = not self.Blocked
        end

        function remote:Ignore()
            self.Ignored = not self.Ignored
        end

        local function ensureBucket(root, idx)
            local bucket = root[idx]
            if not bucket then
                bucket = { types = {}, values = {} }
                root[idx] = bucket
            end
        ...
        function remote:BlockArg(index, value, byType)
            index = tonumber(index)
            if not index or index < 1 then
                return false
            end
            local bucket = ensureBucket(self.BlockedArgs, index)
            if byType then
                bucket.types[tostring(value)] = true
            else
                bucket.values[value] = true
            end
            return true
        end

        function remote:IgnoreArg(index, value, byType)
            index = tonumber(index)
            if not index or index < 1 then
                return false
            end
            local bucket = ensureBucket(self.IgnoredArgs, index)
            if byType then
                bucket.types[tostring(value)] = true
            else
                bucket.values[value] = true
        ...
        local function argsMatch(root, argsPack)
            for i = 1, (argsPack.n or 0) do
                local bucket = root[i]
                if bucket then
                    local value = argsPack[i]
        ...
        function remote:AreArgsBlocked(argsPack)
            return argsMatch(self.BlockedArgs, argsPack)
        end

        function remote:AreArgsIgnored(argsPack)
            return argsMatch(self.IgnoredArgs, argsPack)
        end

        function remote:IncrementCalls(callInfo)
            self.Calls = self.Calls + 1
            table.insert(self.Logs, callInfo)
        end
    ...
    local function getOrCreateRemote(instance)
        local remote = spyState.CurrentRemotes[instance]
        if not remote then
            remote = newRemoteObject(instance)
            spyState.CurrentRemotes[instance] = remote
        end
        return remote
    end

    local function captureRemote(instance, method, argsPack, recorder)
        if typeof(instance) ~= "Instance" then
            return false
        end
        local className = instance.ClassName
        if not remoteMethodByClass[className] then
            return false
        end
        ...
        local argsBlocked = remote:AreArgsBlocked(argsPack)
        local blocked = remote.Blocked or argsBlocked

        if not remote.Ignored and not argsIgnored then
            local hash = tostring(instance) .. "|" .. tostring(method) .. "|" .. tostring(argsPack.n or 0)
            local nowClock = os.clock()
            if spyState.LastCaptureHash ~= hash or (nowClock - (spyState.LastCaptureAt or 0)) > 0.0007 then
                spyState.LastCaptureHash = hash
                spyState.LastCaptureAt = nowClock
                local scriptObj = nil
                if typeof(getCallingScriptFn) == "function" then
                    local okScript, resScript = pcall(function()
                        return getCallingScriptFn((PROTOSMASHER_LOADED ~= nil and 2) or nil)
                    end)
                    if okScript then
                        scriptObj = resScript
                    end
                ...
                local funcObj = nil
                if typeof(getInfoFn) == "function" then
                ...
                local call = {
                    Id = spyState.NextCallId,
                    Method = method,
                    Recorder = recorder,
                    Args = cloneArgs(argsPack),
                    Script = scriptObj,
                    Func = funcObj,
                    TimeUnix = os.time(),
                }
                spyState.NextCallId = spyState.NextCallId + 1
                remote:IncrementCalls(call)
                spyState.Event:Fire(instance, call)
            end
        end
    ...
    local function installHooks()
        if spyState.Hooked then
            return true
        end

        local hookMeta = hookmetamethod or hookMetaMethod
        if not hookMeta and syn and typeof(syn.hookmetamethod) == "function" then
            hookMeta = syn.hookmetamethod
        end
        local hookFunc = hookfunction or hookFunction
        ...
        local newClosure = newcclosure or newCClosure or function(f)
            return f
        end
        local cloneFn = clonefunction or cloneFunction or function(f)
            return f
        end
        ...
        if typeof(hookMeta) == "function" then
            ...
            if okName then
                table.insert(installed, "hookmetamethod(__namecall)")
            else
                table.insert(errors, "__namecall: " .. tostring(errName))
            end
        else
            table.insert(errors, "__namecall: missing hookmetamethod")
        end

        local function installMethod(className, methodName)
            if typeof(hookFunc) ~= "function" then
                table.insert(errors, className .. "." .. methodName .. ": missing hookfunction")
                return
            end
            ...
            local okHook, errHook = pcall(function()
                oldMethod = hookFunc(method, cloneFn(newClosure(function(...)
                    local argc = select("#", ...)
                    local instance = select(1, ...)
                    if typeof(instance) ~= "Instance" then
                        return oldMethod(...)
                    end
                    if instance == spyState.Event or instance.ClassName ~= className then
                        return oldMethod(...)
                    end
                    if not spyState.LogExecutorCalls and typeof(checkCallerFn) == "function" and checkCallerFn() then
                        return oldMethod(...)
                    ...
                    local blocked = false
                    local okCap, capRes = pcall(captureRemote, instance, methodName, argsPack, "__index")
                    if okCap and capRes == true then
                        blocked = true
                    end
                    if blocked then
            ...
            if okHook then
                table.insert(installed, "hookfunction(" .. className .. "." .. methodName .. ")")
            else
                table.insert(errors, className .. "." .. methodName .. ": " .. tostring(errHook))
            end
        end

        installMethod("RemoteEvent", "FireServer")
        installMethod("RemoteFunction", "InvokeServer")
        installMethod("BindableEvent", "Fire")
        installMethod("BindableFunction", "Invoke")
        installMethod("UnreliableRemoteEvent", "FireServer")

        if #installed > 0 then
            spyState.Hooked = true
            spyState.HookDetails = installed
            spyState.HookType = table.concat(installed, " + ")
            spyState.HookError = nil
            return true
        end
        ...
        spyState.HookDetails = installed
        spyState.HookType = nil
        spyState.HookError = (#errors > 0 and table.concat(errors, " | ")) or "No supported hook method"
        return false
    end
    ...
    local function ensureRemotesTab()
        ...
        local hookStatus = callsSection:CreateLabel("Hook: Checking...")
        local statsLabel = callsSection:CreateLabel("Remotes: 0 | Calls: 0")

        callsSection:CreateToggle("RemoteEvent", function(v)
            spyState.RemotesViewing.RemoteEvent = v == true
            queueRefresh()
        end, spyState.RemotesViewing.RemoteEvent)
        callsSection:CreateToggle("RemoteFunction", function(v)
            spyState.RemotesViewing.RemoteFunction = v == true
            queueRefresh()
        end, spyState.RemotesViewing.RemoteFunction)
        callsSection:CreateToggle("BindableEvent", function(v)
            spyState.RemotesViewing.BindableEvent = v == true
            queueRefresh()
        end, spyState.RemotesViewing.BindableEvent)
        callsSection:CreateToggle("BindableFunction", function(v)
            spyState.RemotesViewing.BindableFunction = v == true
            queueRefresh()
        end, spyState.RemotesViewing.BindableFunction)
        callsSection:CreateToggle("UnreliableRemoteEvent", function(v)
            spyState.RemotesViewing.UnreliableRemoteEvent = v == true
            queueRefresh()
        end, spyState.RemotesViewing.UnreliableRemoteEvent)
        callsSection:CreateToggle("Log Executor Calls", function(v)
            spyState.LogExecutorCalls = v == true
        end, spyState.LogExecutorCalls == true)
        ...
        local remoteShell = mk("Frame", {
            Parent = callsSection.Content,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 320),
        })
        local remoteBack = mk("Frame", {
            Parent = remoteShell,
            BackgroundColor3 = C.Control,
            BorderSizePixel = 0,
            Size = UDim2.new(1, 0, 1, 0),
        ...
        corner(remoteBack, 4)
        stroke(remoteBack, C.Stroke, 0.55)
        ...
        local remoteLabel = inspectorSection:CreateLabel("Remote: None")
        local callLabel = inspectorSection:CreateLabel("Call: None")

        local callsShell = mk("Frame", {
            Parent = inspectorSection.Content,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 180),
        })
        local callsBack = mk("Frame", {
            Parent = callsShell,
            BackgroundColor3 = C.Control,
            BorderSizePixel = 0,
            Size = UDim2.new(1, 0, 1, 0),
        ...
        corner(callsBack, 4)
        stroke(callsBack, C.Stroke, 0.55)
        local callsScroll = mk("ScrollingFrame", {
            Parent = callsBack,
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
            Position = UDim2.new(0, 6, 0, 6),
        ...
        local conditionStatus = inspectorSection:CreateDropdown("Condition Status", { "Ignore", "Block" }, "Ignore")
        local conditionMode = inspectorSection:CreateDropdown("Condition Match", { "Value", "Type" }, "Value")
        local conditionIndex = inspectorSection:CreateInput({
            Name = "Arg Index",
            PlaceholderText = "1",
            CurrentValue = "1",
        })
        local conditionValue = inspectorSection:CreateInput({
            Name = "Value / Type",
            PlaceholderText = "string / 10 / true",
            CurrentValue = "",
        })

        local function currentCallCode()
            if not selectedRemote or not selectedCall then
                return "-- Select a call."
            end
            return buildCode(selectedRemote.Instance, selectedCall)
        end

        local function getFilteredRemotes()
            local out = {}
                ...
                if not spyState.Removed[remote.Key] and remote.Instance then
                    ...
                    if spyState.RemotesViewing[className] then
                        if query == "" then
                            ...
                            local name = string.lower(tostring(remote.Instance.Name or ""))
                            local path = string.lower(instancePath(remote.Instance))
                            if string.find(name, query, 1, true) or string.find(path, query, 1, true) then
                                table.insert(out, remote)
                            end
                        end
                    end
            ...
            table.sort(out, function(a, b)
                if (a.Calls or 0) ~= (b.Calls or 0) then
                    return (a.Calls or 0) > (b.Calls or 0)
                end
                return string.lower(tostring(a.Instance.Name or "")) < string.lower(tostring(b.Instance.Name or ""))
            end)
            return out
        end
        ...
        local function refreshInspector()
            if not selectedRemote or not selectedRemote.Instance then
                ...
                callLabel:Set("Call: None")
                codeBox.Text = "-- Select a call."
                return
            end

            local className = tostring(selectedRemote.Instance.ClassName)
            local flags = {}
            if selectedRemote.Blocked then
                table.insert(flags, "blocked")
            end
            if selectedRemote.Ignored then
                table.insert(flags, "ignored")
            end
            local suffix = (#flags > 0) and (" [" .. table.concat(flags, ", ") .. "]") or ""
            remoteLabel:Set(
                string.format(
                    "Remote: %s (%s) | Calls: %d%s",
                    tostring(selectedRemote.Instance.Name or "?"),
                    className,
                    tonumber(selectedRemote.Calls or 0),
                    suffix
                )
            )

            if selectedCall and not table.find(selectedRemote.Logs, selectedCall) then
                selectedCall = nil
            end
            if not selectedCall then
                selectedCall = selectedRemote.Logs[#selectedRemote.Logs]
            end
            if selectedCall then
                local scriptName = (selectedCall.Script and selectedCall.Script.Name) or "UnknownScript"
                callLabel:Set(
                    string.format(
                        "Call #%d | %s | Args: %d | Script: %s | Via: %s",
                        tonumber(selectedCall.Id or 0),
                        tostring(selectedCall.Method or "?"),
                        tonumber((selectedCall.Args and selectedCall.Args.n) or 0),
                        tostring(scriptName),
                        tostring(selectedCall.Recorder or "?")
                    )
                )
                codeBox.Text = currentCallCode()
            else
                callLabel:Set("Call: None")
                codeBox.Text = "-- Remote selected but no calls captured."
            end
        end

        local function rebuildCalls()
            for _, child in ipairs(callsScroll:GetChildren()) do
                if child:IsA("TextButton") or child:IsA("TextLabel") then
                    child:Destroy()
                end
            end
            if not selectedRemote then
                mk("TextLabel", {
                    ...
                    TextXAlignment = Enum.TextXAlignment.Left,
                    TextColor3 = C.SubText,
                    Text = "Select a remote to inspect calls.",
                })
                return
            end
            if #selectedRemote.Logs == 0 then
                mk("TextLabel", {
                    Parent = callsScroll,
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, 24),
                    ...
                    TextXAlignment = Enum.TextXAlignment.Left,
                    TextColor3 = C.SubText,
                    Text = "No calls for selected remote.",
                })
                return
            end
            local maxRows = math.min(#selectedRemote.Logs, 240)
            local order = 0
            for i = #selectedRemote.Logs, math.max(#selectedRemote.Logs - maxRows + 1, 1), -1 do
                order = order + 1
                local call = selectedRemote.Logs[i]
                local row = mk("TextButton", {
                    Parent = callsScroll,
                    BorderSizePixel = 0,
                    Size = UDim2.new(1, 0, 0, 28),
                    BackgroundColor3 = (selectedCall == call) and C.ControlPress or C.Control,
                    Font = FONT,
                    TextSize = 11,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    TextColor3 = C.Text,
                    ...
                    Text = string.format(
                        "#%d  %s  (%d args)  %s",
                        tonumber(call.Id or 0),
                        tostring(call.Method or "?"),
                        tonumber((call.Args and call.Args.n) or 0),
                        os.date("%H:%M:%S", call.TimeUnix or os.time())
                    ),
                    LayoutOrder = order,
                })
                corner(row, 3)
        ...
        local function rebuildRemotes()
            ...
            local remotes = getFilteredRemotes()
            if #remotes == 0 then
                mk("TextLabel", {
                    Parent = remoteScroll,
                    BackgroundTransparency = 1,
            ...
            for idx, remote in ipairs(remotes) do
                local row = mk("TextButton", {
                    ...
                    BorderSizePixel = 0,
                    Size = UDim2.new(1, 0, 0, 30),
                    BackgroundColor3 = (selectedRemote == remote) and C.ControlPress or C.Control,
                    Font = FONT,
                    TextSize = 11,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    TextColor3 = remote.Blocked and Color3.fromRGB(255, 120, 120) or (remote.Ignored and C.SubText or C.Text),
                    AutoButtonColor = false,
                    TextTruncate = Enum.TextTruncate.AtEnd,
                    Text = string.format("[%s] %s  (%d)", tostring(remote.Instance.ClassName), tostring(remote.Instance.Name), tonumber(remote.Calls or 0)),
                    LayoutOrder = idx,
                })
                corner(row, 3)
                stroke(row, C.Stroke, 0.6)
        ...
        callsSection:CreateButton("Clear All Calls", function()
            for _, remote in pairs(spyState.CurrentRemotes) do
                remote:Clear()
            end
            selectedCall = nil
            queueRefresh()
            notify("Remotes", "Cleared all captured calls.", 2.1)
        end)
        callsSection:CreateButton("Restore Removed", function()
            table.clear(spyState.Removed)
            queueRefresh()
            notify("Remotes", "Restored removed remotes.", 2.1)
        end)

        inspectorSection:CreateButton("Run Code", function()
            if not selectedCall then
                notify("Remotes", "Select a call first.", 2.2)
                return
            end
            if typeof(loadstring) ~= "function" then
                notify("Remotes", "loadstring unavailable.", 2.6)
                return
            end
            local okCompile, chunk = pcall(loadstring, currentCallCode())
            if not okCompile or typeof(chunk) ~= "function" then
                notify("Remotes", "Compile failed.", 2.4)
                return
            end
            local okRun, runErr = pcall(chunk)
            if okRun then
                notify("Remotes", "Executed generated code.", 1.9)
            else
        ...
        inspectorSection:CreateButton("Copy Code", function()
            ...
            if not clip and syn and typeof(syn.write_clipboard) == "function" then
                clip = syn.write_clipboard
            end
            if typeof(clip) ~= "function" then
                notify("Remotes", "Clipboard API unavailable.", 2.6)
                return
            end
            local okCopy = pcall(clip, currentCallCode())
            notify("Remotes", okCopy and "Code copied." or "Copy failed.", 2)
        end)
        inspectorSection:CreateButton("Repeat Call", function()
            if not selectedRemote or not selectedCall then
                notify("Remotes", "Select a call first.", 2.2)
                return
            end
            local method = canonicalMethod(selectedCall.Method) or remoteMethodByClass[selectedRemote.Instance.ClassName]
            if not method then
                notify("Remotes", "No valid method.", 2.4)
                return
            end
            local argsPack = selectedCall.Args or { n = 0 }
            local okRun, runErr = pcall(function()
                selectedRemote.Instance[method](selectedRemote.Instance, unpackArgs(argsPack, 1, argsPack.n or 0))
            end)
            if okRun then
                notify("Remotes", "Repeated call.", 2)
            else
                notify("Remotes", "Repeat failed: " .. tostring(runErr), 2.8)
            end
        end)
        inspectorSection:CreateButton("Copy Remote Path", function()
            if not selectedRemote then
                notify("Remotes", "Select a remote first.", 2)
                return
            end
            local clip = setclipboard or toclipboard
            if not clip and syn and typeof(syn.write_clipboard) == "function" then
            ...
            local okCopy = pcall(clip, instancePath(selectedRemote.Instance))
            notify("Remotes", okCopy and "Path copied." or "Copy failed.", 2)
        end)
        inspectorSection:CreateButton("Toggle Ignore Remote", function()
            if not selectedRemote then
                notify("Remotes", "Select a remote first.", 2)
                return
            end
        ...
        inspectorSection:CreateButton("Toggle Block Remote", function()
            if not selectedRemote then
                notify("Remotes", "Select a remote first.", 2)
                return
            end
        ...
        inspectorSection:CreateButton("Clear Selected Remote", function()
            if not selectedRemote then
                notify("Remotes", "Select a remote first.", 2)
                return
            end
        ...
        inspectorSection:CreateButton("Remove Remote From List", function()
            if not selectedRemote then
                notify("Remotes", "Select a remote first.", 2)
                return
            end
            spyState.Removed[selectedRemote.Key] = true
            selectedRemote = nil
            selectedCall = nil
            queueRefresh()
            notify("Remotes", "Removed from list (still runs).", 2.4)
        end)
        inspectorSection:CreateButton("Add Condition", function()
            if not selectedRemote then
                notify("Remotes", "Select a remote first.", 2)
                return
            end
            local status = conditionStatus:GetValue()
            local mode = conditionMode:GetValue()
            local index = tonumber(conditionIndex:GetValue())
            local valueRaw = conditionValue:GetValue()
            if not index or index < 1 then
                notify("Remotes", "Arg Index must be >= 1.", 2.4)
                return
            end
            local byType = mode == "Type"
            local parsedValue = valueRaw
            if not byType then
            ...
            if status == "Block" then
                okSet = selectedRemote:BlockArg(index, parsedValue, byType)
            else
                okSet = selectedRemote:IgnoreArg(index, parsedValue, byType)
            end
            if okSet then
                notify("Remotes", status .. " condition added.", 2)
            else
                notify("Remotes", "Failed to add condition.", 2.4)
            end
        end)
        ...
        local hookOk = installHooks()
        if hookOk then
            hookStatus:Set("Hook: Active (" .. tostring(spyState.HookType or "unknown") .. ")")
        else
            hookStatus:Set("Hook: Failed - " .. tostring(spyState.HookError or "unknown"))
            notify("Remotes", "Hook failed: " .. tostring(spyState.HookError or "unknown"), 4)
        end

        local eventConnection = spyState.Event.Event:Connect(function(remoteInstance)
            local remote = spyState.CurrentRemotes[remoteInstance]
            if remote and spyState.Removed[remote.Key] then
                return
            end
            queueRefresh()
        end)
        track(self.Connections, eventConnection)
        track(self.Connections, remotesTab.Page.Destroying:Connect(function()
            if eventConnection and eventConnection.Disconnect then
    ...
    infoSection:CreateParagraph("Developer Tabs", "Hydroxide-style remote spy integrated.")
    infoSection:CreateLabel("Remotes: capture, inspect, replay, block/ignore, conditions")
