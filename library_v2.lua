...
function Window:CreateUniversalCategory(options)
    ...
    local oldSpy = sharedGlobals.__LIMBO_REMOTE_SPY_STATE
    if type(oldSpy) == "table" and typeof(oldSpy.UninstallHooks) == "function" then
        pcall(oldSpy.UninstallHooks)
    end
    sharedGlobals.__LIMBO_REMOTE_SPY_STATE = nil
    UILibrary._remoteSpyState = nil
    ...
    local function ensureRemotesTab()
        ...
        remotesMain:CreateParagraph("Remotes", "No methods are currently available.")
        remotesMain:CreateLabel("Remotes tab is intentionally empty.")
        return remotesTab
    end
    ...
    infoSection:CreateParagraph("Developer Tabs", "Remotes tab has been cleared.")
    infoSection:CreateLabel("Remotes: empty")
