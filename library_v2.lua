...
function Window:CreateUniversalCategory(options)
    ...
    local simpleSpy = UILibrary._simpleSpyState
    ...
    local function serializeValue(value, depth, seen)
        ...
        elseif valueType == "table" then
            ...
            local isArray = not hasNonArrayKey
            if isArray then
                for i = 1, maxIndex do
            ...
            for k, v in pairs(value) do
                ...
                keyed[#keyed + 1] = keyCode .. " = " .. serializeValue(v, depth + 1, seen)
            end
            table.sort(keyed)
            seen[value] = nil
            return "{ " .. table.concat(keyed, ", ") .. " }"
        end
    ...
    local function buildRemoteCode(remote, method, argsPack, entryId)
        ...
        if canonicalMethod == "InvokeServer" then
            ...
            lines[#lines + 1] = "    remote:FireServer(unpackFn(callArgs, 1, args.n))"
        end
        lines[#lines + 1] = "end"
    ...
    local function emitSpyUpdate(entry)
        for token, callback in pairs(simpleSpy.Listeners) do
            if typeof(callback) == "function" then
                task.defer(callback, entry)
            else
                simpleSpy.Listeners[token] = nil
            end
        end
    end
    ...
    local function captureRemote(remote, method, argsPack, recorderType)
        ...
        if #simpleSpy.Logs > (simpleSpy.MaxLogs or 450) then
            table.remove(simpleSpy.Logs, #simpleSpy.Logs)
        end

        emitSpyUpdate(entry)
    end

    local function installSimpleSpyHook()
        if simpleSpy.Hooked then
            return true
        end
    ...
    local function ensureRemotesTab()
        ...
        local codeBox = mk("TextBox", {
            ...
            TextColor3 = C.Text,
            PlaceholderColor3 = C.SubText,
            Text = "-- Select a remote call to inspect generated code.",
        })
        pcall(function()
        ...
        local function getSelectedEntry()
            ...
            for _, entry in ipairs(simpleSpy.Logs) do
        ...
        local function excludedCount()
            local n = 0
            for _ in pairs(simpleSpy.Excluded) do
                n = n + 1
            end
            return n
        end
        ...
        local function rebuildList()
            ...
            local displayCount = math.min(#simpleSpy.Logs, 220)
        ...
        callsSection:CreateButton("Clear List", function()
            table.clear(simpleSpy.Logs)
            selectedEntryId = nil
            emitSpyUpdate(nil)
            queueRefresh()
            notify("Remotes", "Captured list cleared.", 2)
        end)

        callsSection:CreateButton("Clear Exclude List", function()
            table.clear(simpleSpy.Excluded)
            emitSpyUpdate(nil)
            queueRefresh()
        ...
        inspectorSection:CreateButton("Run Code", function()
            ...
            if typeof(loadstring) ~= "function" then
                notify("Remotes", "loadstring is not available in this executor.", 3.1)
                return
            end

            local okCompile, chunkOrErr = pcall(loadstring, entry.Code or "")
            if not okCompile or type(chunkOrErr) ~= "function" then
                notify("Remotes", "Failed to compile selected code.", 2.8)
                return
            end
            ...
            notify("Remotes", "Selected code executed.", 2)
        end)
        ...
        inspectorSection:CreateButton("Copy Code", function()
            ...
            if okCopy then
                notify("Remotes", "Code copied to clipboard.", 2)
            else
                notify("Remotes", "Failed to copy code.", 2.6)
            end
        end)
        ...
        track(self.Connections, remotesTab.Page.Destroying:Connect(function()
            simpleSpy.Listeners[listenerToken] = nil
        end))
