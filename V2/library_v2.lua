...
local function stroke(x, color, trans, thick)
...
local function addRailGlyph(parent, active, colors)
    local theme = colors or C
    ...
    for row = 0, 1 do
        for col = 0, 1 do
            local dot = mk("Frame", {
                Parent = holder,
                BackgroundColor3 = active and theme.Accent or theme.SubText,
                BorderSizePixel = 0,
...
function Window:SetVisible(v)
    ...
    if shouldShow then
        ...
        self.AnimScaleTween = tw(self.MainScale, 0.4, { Scale = 1 }, Enum.EasingStyle.Exponential)
        self.AnimFadeTween = tw(self.Main, 0.3, { BackgroundTransparency = 0 })
        self.AnimStrokeTween = tw(self.MainStroke, 0.3, { Transparency = 0.2 })
...
function Window:PlayInitializeAnimation()
    ...
    local col = self.Colors or C
    local overlay = mk("Frame", {
        Parent = self.Main,
        BackgroundColor3 = col.Main,
        BorderSizePixel = 0,
    ...
    mk("UIGradient", {
        ...
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, col.Main),
            ColorSequenceKeypoint.new(1, col.Top),
        }),
...
function Window:UpdateHeader()
    ...
    local titleText = activeTab and tostring(activeTab.Name or "") or "Overview"
    local subText = activeSubTab and tostring(activeSubTab.Name or "") or ""
    
    if self.HeaderTitleLabel.Text ~= titleText then
        tw(self.HeaderTitleLabel, 0.2, { TextTransparency = 1 }):Play()
        task.delay(0.2, function()
            self.HeaderTitleLabel.Text = titleText
            tw(self.HeaderTitleLabel, 0.2, { TextTransparency = 0 }):Play()
        end)
    end
    
    if self.HeaderSubtitleLabel and self.HeaderSubtitleLabel.Text ~= subText then
        tw(self.HeaderSubtitleLabel, 0.2, { TextTransparency = 1 }):Play()
        task.delay(0.2, function()
            self.HeaderSubtitleLabel.Text = subText
            tw(self.HeaderSubtitleLabel, 0.2, { TextTransparency = 0 }):Play()
        end)
    end
end
...
function Window:SelectTab(tabOrName)
    ...
    for _, t in ipairs(self.Tabs) do
        ...
        local col = self.Colors or C
        tw(t.Indicator, 0.25, { BackgroundTransparency = active and 0 or 1, Size = active and UDim2.new(0, 3, 1, -16) or UDim2.new(0, 3, 0, 0) }, Enum.EasingStyle.Quart):Play()
        tw(t.ButtonBack, 0.25, { BackgroundTransparency = active and 0.15 or 1 }):Play()
        tw(t.Label, 0.25, { TextColor3 = active and col.Text or col.SubText, Position = active and UDim2.new(0, 38, 0, 0) or UDim2.new(0, 34, 0, 0) }):Play()
        if t.Glyph then
            for _, child in ipairs(t.Glyph:GetChildren()) do
                if child:IsA("Frame") then
                    tw(child, 0.25, { BackgroundColor3 = active and col.Accent or col.SubText }):Play()
                end
            end
        end
    end
...
function Window:CreatePlayersCategory(options)
    ...
    local listBack = mk("Frame", {
        Parent = listShell,
        BackgroundColor3 = col.PanelInset,
        BorderSizePixel = 0,
        Size = UDim2.new(1, 0, 1, 0),
    })
    corner(listBack, 8)
    stroke(listBack, col.Stroke, 0.45)
    ...
    local function refreshPlayerList()
        ...
        for _, p in ipairs(filtered) do
            ...
            local row = mk("TextButton", {
                ...
                Text = displayText,
                BackgroundColor3 = (selectedPlayer == p) and col.ControlPress or col.Control,
            })
            corner(row, 6)
            stroke(row, col.Stroke, 0.6)
            mk("UIPadding", {
                Parent = row,
            ...
            track(self.Connections, row.MouseEnter:Connect(function()
                if selectedPlayer ~= p then
                    tw(row, 0.1, { BackgroundColor3 = col.ControlHover }):Play()
                end
            end))
            track(self.Connections, row.MouseLeave:Connect(function()
                tw(row, 0.1, { BackgroundColor3 = (selectedPlayer == p) and col.ControlPress or col.Control }):Play()
            end))
...
function Window:CreateRemotesCategory(options)
    ...
    stroke(scriptShell, col.Stroke, 0.55)
...
local function controlBack(parent, h, colors)
    ...
    corner(b, 8)
    stroke(b, col.Stroke, CONTROL_STROKE_TRANSPARENCY)
    return b
end
...
local function addControlDivider(parent, colors)
    local col = colors or C
    return mk("Frame", {
        Parent = parent,
...
function Section:CreateButton(a, b)
    ...
    track(self.Window.Connections, btn.MouseEnter:Connect(function()
        tw(btn, 0.15, { BackgroundColor3 = buttonHoverColor }):Play()
        tw(scale, 0.15, { Scale = 1.015 }):Play()
    end))
    track(self.Window.Connections, btn.MouseLeave:Connect(function()
        tw(btn, 0.15, { BackgroundColor3 = buttonBaseColor }):Play()
        tw(scale, 0.15, { Scale = 1 }):Play()
    end))
    track(self.Window.Connections, btn.MouseButton1Down:Connect(function()
        tw(btn, 0.1, { BackgroundColor3 = buttonPressColor }):Play()
        tw(scale, 0.1, { Scale = 0.97 }):Play()
    end))
    track(self.Window.Connections, btn.MouseButton1Up:Connect(function()
        tw(scale, 0.15, { Scale = 1.015 }):Play()
    end))
...
function Section:CreateToggle(a, b, c)
    ...
    corner(switch, 99)
    local switchStroke = stroke(switch, col.Stroke, 0.7)
    ...
    addControlDivider(shell, col)

    local controller = { Frame = shell }
    function controller:Set(v, skip)
        value = v == true
        tw(switch, 0.15, { BackgroundColor3 = value and col.Accent or CONTROL_FIELD }, Enum.EasingStyle.Quart):Play()
        ...
        tw(knob, 0.15, {
            ...
            BackgroundColor3 = value and Color3.new(1, 1, 1) or col.Text
...
function Section:CreateSlider(a, b, c, d, e)
    ...
    local shell = controlShell(self, 68)
    local back = controlBack(shell, 68, col)
...
function Section:CreateDropdown(a, b, c, d)
    ...
    function controller:SetOpen(v)
        ...
        if open then
            ...
            menu.Visible = true
            tw(arrow, 0.15, { Rotation = 180 }):Play()
            local h = (#opts * 28) + 10
            tw(shell, 0.2, { Size = UDim2.new(1, 0, 0, 40 + 6 + h) }, Enum.EasingStyle.Quart, Enum.EasingDirection.Out):Play()
            tw(menu, 0.2, { Size = UDim2.new(1, 0, 0, h), BackgroundTransparency = 0 }, Enum.EasingStyle.Quart, Enum.EasingDirection.Out):Play()
        else
            OPEN_DROPDOWNS[self] = nil
            tw(arrow, 0.15, { Rotation = 0 }):Play()
            tw(shell, 0.2, { Size = UDim2.new(1, 0, 0, 40) }, Enum.EasingStyle.Quart, Enum.EasingDirection.Out):Play()
            tw(menu, 0.15, { Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1 }, Enum.EasingStyle.Quart, Enum.EasingDirection.Out):Play()
...
function Section:CreateInput(a, b, c)
    ...
    local col = self.Window.Colors or C
    local shell = controlShell(self, 40)
    local back = controlBack(shell, 40, col)
    mk("TextLabel", {
        Parent = back,
        BackgroundTransparency = 1,
    ...
    corner(inBack, 8)
    local inStroke = stroke(inBack, col.Stroke, 0.7)
    ...
    track(self.Window.Connections, box.Focused:Connect(function()
        tw(inBack, 0.2, { BackgroundColor3 = CONTROL_FIELD_HOVER }):Play()
        tw(inStroke, 0.2, { Transparency = 0.4 }):Play()
    end))
    track(self.Window.Connections, box.FocusLost:Connect(function(enter)
        ...
        tw(inBack, 0.2, { BackgroundColor3 = CONTROL_FIELD }):Play()
        tw(inStroke, 0.2, { Transparency = 0.7 }):Play()
    end))
    bindSurfaceHover(self.Window, back, CONTROL_SURFACE, CONTROL_SURFACE_HOVER)
    addControlDivider(shell, col)
    return attachControlTooltip(self, controller, back, tooltipText)
...
function Section:CreateKeybind(a, b, c)
    ...
    corner(btn, 8)
    local btnStroke = stroke(btn, col.Stroke, 0.7)
    ...
    track(self.Window.Connections, btn.MouseButton1Click:Connect(function()
        listening = true
        btn.Text = "..."
        tw(btn, 0.15, { BackgroundColor3 = CONTROL_FIELD_HOVER }):Play()
        tw(btnStroke, 0.15, { Transparency = 0.4 }):Play()
    end))
    track(self.Window.Connections, UIS.InputBegan:Connect(function(i, gpe)
        ...
        if listening then
            ...
            tw(btn, 0.15, { BackgroundColor3 = CONTROL_FIELD }):Play()
            tw(btnStroke, 0.15, { Transparency = 0.7 }):Play()
            return
        end
...
function Section:CreateParagraph(a, b)
    ...
    corner(shell, 8)
    stroke(shell, col.Stroke, CONTROL_STROKE_TRANSPARENCY)
    mk("UIPadding", {
        Parent = shell,
...
local function createSectionForSubTab(tab, subTab, a, b)
    ...
    local col = tab.Window.Colors or C
...
function Tab:SelectSubTab(subTabOrName)
    ...
    for _, subTab in ipairs(self.SubTabs or {}) do
        ...
        tw(subTab.Label, 0.2, { TextColor3 = active and col.Text or col.SubText }):Play()
...
function Window:CreateTab(a, _iconMaybe)
    ...
    local back = mk("Frame", {
        Parent = btn,
        BackgroundColor3 = col.SidebarActive,
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Size = UDim2.new(1, 0, 1, 0),
    })
    ...
    corner(ind, 99)
    local glyph = addRailGlyph(btn, false, self.Colors)
    ...
    track(self.Connections, btn.MouseEnter:Connect(function()
        if self.ActiveTab ~= tab then
            ...
            for _, child in ipairs(glyph:GetChildren()) do
                if child:IsA("Frame") then
                    child.BackgroundColor3 = col.Text
                end
            end
        end
    end))
    track(self.Connections, btn.MouseLeave:Connect(function()
        if self.ActiveTab ~= tab then
            ...
            for _, child in ipairs(glyph:GetChildren()) do
                if child:IsA("Frame") then
                    child.BackgroundColor3 = col.SubText
                end
            end
        end
    end))
...
function UILibrary:CreateWindow(arg)
    ...
    local colors = setmetatable({}, { __index = C })
    local colorKeys = {
        "Main","Top","Sidebar","SidebarActive","Panel","PanelInset",
        "Control","ControlHover","ControlPress","Stroke","Accent",
        "Text","SubText","Success","Warning","Error"
    }
    if typeof(o.Colors) == "table" then
        for _, key in ipairs(colorKeys) do
            if o.Colors[key] ~= nil then
                colors[key] = o.Colors[key]
            end
        end
    end
    for _, key in ipairs(colorKeys) do
    ...
    corner(close, 6)
    stroke(close, colors.Stroke, 0.7)
    ...
    stroke(tooltipFrame, colors.Stroke, 0.3)
    mk("UIPadding", {
        Parent = tooltipFrame,
...
function UILibrary:Notify(args)
    ...
    local col = self._window and self._window.Colors or C
    ...
    local toastStroke = stroke(toast, col.Stroke, 0.4)
    ...
    tw(toast, 0.4, { BackgroundTransparency = 0.05 }, Enum.EasingStyle.Quart):Play()
    tw(toastStroke, 0.4, { Transparency = 0.4 }, Enum.EasingStyle.Quart):Play()
    tw(pop, 0.5, { Scale = 1 }, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
    tw(t, 0.3, { TextTransparency = 0 }):Play()
    tw(c, 0.3, { TextTransparency = 0 }):Play()
    tw(timerTrack, 0.3, { BackgroundTransparency = 0.8 }):Play()
    tw(timerFill, 0.3, { BackgroundTransparency = 0 }):Play()

    task.delay(0.1, function()
        tw(timerFill, duration, { Size = UDim2.new(0, 0, 1, 0) }, Enum.EasingStyle.Linear):Play()
    end)

    task.delay(duration, function()
        tw(pop, 0.3, { Scale = 0.9 }, Enum.EasingStyle.Back, Enum.EasingDirection.In):Play()
        tw(toast, 0.3, { BackgroundTransparency = 1 }, Enum.EasingStyle.Quart):Play()
        tw(toastStroke, 0.3, { Transparency = 1 }, Enum.EasingStyle.Quart):Play()
        tw(t, 0.2, { TextTransparency = 1 }):Play()
        tw(c, 0.2, { TextTransparency = 1 }):Play()
