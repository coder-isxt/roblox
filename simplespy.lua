...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "Remotes",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
local simpleSpyHost = type(getgenv().__SimpleSpyHost) == "table" and getgenv().__SimpleSpyHost or nil
local hostMode = simpleSpyHost ~= nil
local menuLibrary
local menuWindow
local menuLogSection
local menuActionSection
local menuSettingsSection
local libraryMenuEnabled = false
...
function eventSelect(frame)
    if selected and selected.Log  then
        if selected.Button then
            spawn(function()
                local deselectColor = libraryMenuEnabled and Color3.fromRGB(20, 29, 41) or Color3.fromRGB(0, 0, 0)
...
local function loadLibraryV2()
    if menuLibrary then
        return menuLibrary
    end

    local loaded
    if loadfile and isfile and isfile("copy/library_v2.lua") then
        local chunk = loadfile("copy/library_v2.lua")
        if chunk then
    ...
    if not loaded and readfile and isfile and isfile("copy/library_v2.lua") then
        local ok, source = pcall(readfile, "copy/library_v2.lua")
        if ok and source and loadstring then
            local compiled = loadstring(source)
            if compiled then
                local okLib, result = pcall(compiled)
                if okLib then
                    loaded = result
                end
            end
...
local function ensureLibraryMenu()
    if libraryMenuAttempted then
        return libraryMenuEnabled
    end
    libraryMenuAttempted = true

    if libraryMenuEnabled and menuWindow then
        return true
    end

    if hostMode then
        if simpleSpyHost.LogSection and simpleSpyHost.ActionSection and simpleSpyHost.SettingsSection then
            menuLibrary = simpleSpyHost.Library
            menuWindow = simpleSpyHost.Window
        ...
        libraryMenuError = "host mode missing required sections"
        return false
    end

    local lib = loadLibraryV2()
    if not lib or type(lib.CreateWindow) ~= "function" then
        libraryMenuError = "library_v2 did not return a valid window API"
        return false
    end

    local ok, windowOrErr = pcall(function()
    ...
    if not ok or not windowOrErr then
        libraryMenuError = tostring(windowOrErr)
        warn("[SimpleSpy] Failed to create library_v2 menu:", windowOrErr)
        return false
    end

    menuWindow = windowOrErr
    local spyTab = menuWindow:CreateTab({
        Name = "Spy",
        Icon = "scripts",
    })
...
function newButton(name, description, onClick)
    if not ensureLibraryMenu() then
        warn("[SimpleSpy] library_v2 menu unavailable:", libraryMenuError or "unknown error")
        return
    end
...
local function requireLibraryMenu()
    if ensureLibraryMenu() then
        return
    end
    error("SimpleSpy requires copy/library_v2.lua for its menu. Error: " .. tostring(libraryMenuError))
end
...
local function shutdown()
    ...
    clear(remoteLogs)
    disablehooks()
    if menuOwnedBySimpleSpy and menuWindow and menuWindow.Destroy then
        pcall(function()
            menuWindow:Destroy()
        end)
    end
    if SimpleSpy3 and SimpleSpy3.Parent then
        SimpleSpy3:Destroy()
    end
    Storage:Destroy()
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        ...
        requireLibraryMenu()
        if hostMode and type(simpleSpyHost.SetCode) == "function" and type(simpleSpyHost.GetCode) == "function" then
            codebox = {
                setRaw = function(_, text)
                    simpleSpyHost:SetCode(tostring(text or ""))
                end,
                getString = function()
                    return tostring(simpleSpyHost:GetCode() or "")
                end
            }
        else
...
function()
    ...
    if func then
        ...
        codebox:setRaw("-- Calling function info\n-- Generated by the simple serializer\n\n"..selected.Function)
        TextLabel.Text = "Done! Function info generated by the simple Serializer."
    else
...
newButton(
    ...
    function()
        ...
        for i,v in next, logs do
            if v.Log and typeof(v.Log) == "Instance" then
                v.Log:Destroy()
            end
        end
        clear(logs)
